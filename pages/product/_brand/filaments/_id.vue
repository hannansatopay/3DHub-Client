<template>
    <div>
        <!-- <ul v-for="(data,index) in randomProducts" :key="index"><li>{{data}}</li></ul> -->
        ...
        <v-container>
            ...
            <v-row>
                <v-col cols="12">
                    <h4 class="mb-5">Other Recommended Filaments</h4>
                    <client-only>
...
export default {
    ...
    computed: {
        ...
        randomProducts(){
            return _.shuffle(_.filter(_.reject(this.$store.getters.getProducts, {"name": this.$route.params.id}), {product_type: 'Filaments'}), 8)
        },
    }
}
</script>
