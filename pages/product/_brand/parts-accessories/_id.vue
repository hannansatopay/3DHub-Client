<template>
    <div>
        ...
        <v-container>
            ...
            <v-row>
                <v-col cols="12">
                    <h4 class="mb-5 mt-5">Other Recommended Part & Accessories</h4>
                    <client-only>
...
export default {
    ...
    components: {
        VueSlickCarousel,SearchToolBar, ProductDetail, ProductTile
    },
    methods: {
        goBack(){
            window.history.back();
    ...
    computed: {
        getProductInfo(){
            ...
            query['product_type'] = 'Parts & Accessories'
        ...
        randomProducts(){
            return _.shuffle(_.filter(_.reject(this.$store.getters.getProducts, {"name": this.$route.params.id}), {product_type: 'Parts & Accessories'}), 8)
        }
    }
}
</script>
