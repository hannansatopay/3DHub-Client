...
export default {
    ...
    computed: {
        getProductInfo(){
            ...
            query['product_type'] = 'Printer'
            let url = this.$router.history.current.path.split('/');
            if(url !== '' && url.length > 2){
               // query['brand'] = url[2]
               query['brand'] = decodeURI(url[2]) 
            }
            return _.filter(this.$store.getters.getProducts, query)
        },
        randomProducts(){
            return _.shuffle(_.filter(_.reject(this.$store.getters.getProducts, {"name": this.$route.params.id}), {"product_type": 'Printer'}), 8)
        },
    }
}
</script>
